	...
	const FStrategicSimulationConfig& Config)
{
	...
	{
		Snapshot.bCanAdvanceTime = false;
		if (!CommandSequenceValidation.Diagnostics.IsEmpty())
		{
	...
	{
		Snapshot.bCanAdvanceTime = false;
		if (!ProjectStateValidation.Diagnostics.IsEmpty())
		{
			const FString& Diagnostic = ProjectStateValidation.Diagnostics[0].Message;
			if (!Snapshot.Diagnostics.ContainsByPredicate(
				[&Diagnostic](const FString& Existing) { return Existing == Diagnostic; }))
			{
				Snapshot.Diagnostics.Add(Diagnostic);
	...
	{
		const FStrategicCommandResult PersonnelStateValidation =
			FStrategicCommandService::ValidateStrategicPersonnelState(Campaign, Rules);
		if (!PersonnelStateValidation.bAccepted)
		{
			Snapshot.bCanAdvanceTime = false;
			if (!PersonnelStateValidation.Diagnostics.IsEmpty())
			{
				const FString& Diagnostic = PersonnelStateValidation.Diagnostics[0].Message;
				if (!Snapshot.Diagnostics.ContainsByPredicate(
					[&Diagnostic](const FString& Existing) { return Existing == Diagnostic; }))
				{
					Snapshot.Diagnostics.Add(Diagnostic);
	...
	const FStrategicCommandResult AdversaryStateValidation =
		FStrategicCommandService::ValidateStrategicAdversaryState(Campaign, Rules, Config);
	if (!AdversaryStateValidation.bAccepted)
	{
		Snapshot.bCanAdvanceTime = false;
		if (!AdversaryStateValidation.Diagnostics.IsEmpty())
		{
			const FString& Diagnostic = AdversaryStateValidation.Diagnostics[0].Message;
			if (!Snapshot.Diagnostics.ContainsByPredicate(
				[&Diagnostic](const FString& Existing) { return Existing == Diagnostic; }))
			{
				Snapshot.Diagnostics.Add(Diagnostic);
	...
	if (AdversaryStateValidation.bAccepted)
	{
		const FStrategicCommandResult AdversaryCapacityValidation =
			FStrategicCommandService::ValidateStrategicTimeAdvanceAdversaryCapacity(
				Campaign, Rules, Config, MaximumTimeAdvanceSeconds);
		if (!AdversaryCapacityValidation.bAccepted)
		{
			Snapshot.bCanAdvanceTime = false;
			if (!AdversaryCapacityValidation.Diagnostics.IsEmpty())
			{
				const FString& Diagnostic = AdversaryCapacityValidation.Diagnostics[0].Message;
				if (!Snapshot.Diagnostics.ContainsByPredicate(
					[&Diagnostic](const FString& Existing) { return Existing == Diagnostic; }))
				{
					Snapshot.Diagnostics.Add(Diagnostic);
	...
	if (TimeAdvanceConfigValidation.bAccepted)
	{
		const FStrategicCommandResult MonthlyFinancialCapacityValidation =
			FStrategicCommandService::ValidateStrategicTimeAdvanceFinancialCapacity(
				Campaign, Rules, Config, MaximumTimeAdvanceSeconds);
		if (!MonthlyFinancialCapacityValidation.bAccepted)
		{
			Snapshot.bCanAdvanceTime = false;
			if (!MonthlyFinancialCapacityValidation.Diagnostics.IsEmpty())
			{
				const FString& Diagnostic = MonthlyFinancialCapacityValidation.Diagnostics[0].Message;
				if (!Snapshot.Diagnostics.ContainsByPredicate(
					[&Diagnostic](const FString& Existing) { return Existing == Diagnostic; }))
				{
					Snapshot.Diagnostics.Add(Diagnostic);
	...
	for (const FStrategicBaseState& Base : Campaign.Bases)
	{
		...
		View.bInfrastructureValid = bInfrastructureValid;
